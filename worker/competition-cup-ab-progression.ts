...
async function validateTarget(env: Env, source: StageRow, targetStageId: number, roundLinkId: number): Promise<TargetValidation> {
  ...
  if (Number(encounters?.total ?? 0) > 0) return { ok: false, error: 'La etapa destino ya tiene cruces sorteados' };
...
async function stageWinners(env: Env, stageId: number): Promise<WinnersResult> {
  const encounters = await env.DB.prepare(
    `SELECT ce.winner_entry_id, ce.status, e.display_name
  ...
  ).bind(stageId).all<{ winner_entry_id: number | null; status: string; display_name: string | null }>();
...
async function insertPairs(env: Env, targetStageId: number, roundLinkId: number, pairs: Array<[number, number | null]>, prefix: string) {
...
async function drawRoundOf16(env: Env, user: SessionUser, groupStage: StageRow, targetStageId: number, roundLinkId: number) {
  ...
  await audit(env, user.id, 'competition.cup_ab_r16_drawn', String(targetStageId), {
    sourceGroupStageId: groupStage.id, randomSeed, restriction: 'SECOND_VS_THIRD', pairs,
  });
  return json({ ok: true, action: 'DRAW_R16', randomSeed, pairs });
}

async function drawQuarterfinals(env: Env, user: SessionUser, r16Stage: StageRow, groupStageId: number, targetStageId: number, roundLinkId: number) {
  ...
  const randomSeed = freshSeed();
  const pairs = pairSequential(shuffled(pool, mulberry32(randomSeed)));
  ...
  await audit(env, user.id, 'competition.cup_ab_qf_drawn', String(targetStageId), {
    sourceGroupStageId: groupStageId, sourceRoundOf16StageId: r16Stage.id, randomSeed, pairs,
  });
...
async function drawWinnersStage(
  ...
  await audit(env, user.id, action === 'DRAW_SEMIS' ? 'competition.cup_ab_semis_drawn' : 'competition.cup_ab_final_built', String(targetStageId), {
    sourceStageId: sourceStage.id, randomSeed, pairs,
  });
...
export async function handleCompetitionCupAbProgression(request: Request, env: Env): Promise<Response | null> {
  ...
  const body = await request.json().catch(() => null) as { targetStageId?: number; roundLinkId?: number; groupStageId?: number } | null;
  ...
  if (action === 'r16') return drawRoundOf16(env, user, sourceStage, targetStageId, roundLinkId);
  ...
  if (action === 'semifinals') return drawWinnersStage(env, user, sourceStage, targetStageId, roundLinkId, 'DRAW_SEMIS');
