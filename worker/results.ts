...
export async function syncRoundResults(roundId: number, env: Env) {
  ...
  const rows = await env.DB.prepare(
    `SELECT id, provider_fixture_id
     ...
     ORDER BY id`,
  ).bind(roundId).all<{ id: number; provider_fixture_id: string }>();

  const matches = rows.results ?? [];
  if (matches.length === 0) throw new Error('La fecha no tiene partidos de API-Football');
  if (matches.length > 20) throw new Error('API-Football permite actualizar hasta 20 partidos por consulta');

  const endpoint = new URL('https://v3.football.api-sports.io/fixtures');
  endpoint.searchParams.set('ids', matches.map((match) => match.provider_fixture_id).join('-'));
  ...
  if (!response.ok || !data) throw new Error('No se pudo consultar API-Football');
  const details = apiErrors(data.errors);
  if (details) throw new Error(`API-Football: ${details}`);

  const localByFixture = new Map(matches.map((match) => [match.provider_fixture_id, match.id]));
  let updated = 0;
  let finalized = 0;

  for (const item of data.response ?? []) {
    const matchId = localByFixture.get(String(item.fixture.id));
    if (!matchId) continue;
    ...
    updated += 1;
    if (final || isVoid) finalized += 1;
  }

  const calculated = await calculateScores(roundId, env);
  return { updated, finalized, calculated, requestCount: 1 };
}
     ...
     WHERE result_finalized_at IS NULL
       ...
       AND (
         ...
         (julianday(kickoff_at) <= julianday('now', '+10 minutes')
          AND julianday(kickoff_at) >= julianday('now', '-3 hours'))
       )`,
  ).all<{ round_id: number }>();

  for (const row of rounds.results ?? []) {
    try {
      await syncRoundResults(row.round_id, env);
    } catch (caught) {
      console.error('Scheduled result sync failed', row.round_id, caught);
    }
  }
}
