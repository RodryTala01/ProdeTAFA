...
export default function AdminRounds() {
  ...
  useEffect(() => {
    void loadRounds().catch((caught) => {
      setError(caught instanceof Error ? caught.message : 'No se pudieron cargar las fechas');
    });
  }, []);
  ...
  async function addFixture(fixture: Fixture, matchType: 'NORMAL' | 'PENALTIES_ONLY') {
    ...
    setSuccess('');
    try {
      await api<{ ok: true }>(`/api/admin/rounds/${selected.id}/matches`, {
        method: 'POST',
        body: JSON.stringify({ fixture, matchType }),
      });
  ...
  async function removeMatch(match: StoredMatch) {
    ...
    setSuccess('');
    try {
      await api<{ ok: true }>(`/api/admin/rounds/${selected.id}/matches/${match.id}`, { method: 'DELETE' });
    ...
    <div className="rounds-layout">
      <aside className="card rounds-sidebar">
        <div className="rounds-title-row">
          <div>
            <span className="eyebrow">FECHAS</span>
            <h2>Fechas del Prode</h2>
          </div>
        ...
        <div className="round-list">
          {rounds.map((round) => (
            <button
              type="button"
              className={`round-item ${selected?.id === round.id ? 'round-item--active' : ''}`}
              key={round.id}
              onClick={() => void loadRound(round.id)}
            >
              <span><strong>{round.name}</strong><small>{round.status === 'draft' ? 'Borrador' : round.status}</small></span>
      ...
      <section className="round-main">
        ...
        {!selected ? (
          <section className="card empty-round">
            <span className="empty-icon">⚽</span>
            <h2>Creá o elegí una fecha</h2>
            <p>Después vas a poder buscar los partidos reales y agregarlos sin cargar sus datos manualmente.</p>
          </section>
        ) : (
          <>
            <section className="card round-header-card">
              <div>
                ...
                <h1>{selected.name}</h1>
                <p>{selected.matches.length} de 12 partidos cargados.</p>
              </div>
                ...
                <div className="stored-match-list">
                  {selected.matches.map((match) => (
                    <div className="stored-match" key={match.id}>
                      ...
                      <button className="button button--ghost button--danger" disabled={loading} onClick={() => void removeMatch(match)}>Quitar</button>
                    </div>
                  ))}
                </div>
              </section>
                        ...
                        return (
                          <article className="fixture-card" key={fixture.providerFixtureId}>
                            ...
                            <div className="fixture-versus">
                              <Team name={fixture.home.name} logoUrl={fixture.home.logoUrl} />
                              <span>VS</span>
                              <Team name={fixture.away.name} logoUrl={fixture.away.logoUrl} />
                            </div>
                            <div className="fixture-actions">
                              {alreadyAdded ? (
                                <span className="added-badge">✓ Ya agregado</span>
                              ) : (
                                <>
                                  <button className="button button--primary" disabled={loading} onClick={() => void addFixture(fixture, 'NORMAL')}>Agregar</button>
                                  <button className="button button--secondary" disabled={loading} onClick={() => void addFixture(fixture, 'PENALTIES_ONLY')}>Agregar como PENALES</button>
                                </>
                              )}
                            </div>
                          </article>
