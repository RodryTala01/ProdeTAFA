            ...
            <div className={selectedCompetitionId === null ? "competition-cards" : "form-stack"}>
              {selected.competitions.filter((competition) => selectedCompetitionId === null || competition.id === selectedCompetitionId).map((competition) => (
                <div className="card panel" key={competition.id}>
                  ...
                  {competition.family !== 'LEAGUE' && !['COPA_A','COPA_B','COPA_TOTAL'].includes(competition.code) && <p className="competition-pending">Configuración deportiva específica pendiente. El armado de grupos, parejas y cruces tendrá opciones asistida y manual, con validaciones y auditoría.</p>}
                  ...
                  <details open={!competition.stages.some(s=>['ACCUMULATIVE_GROUPS','ROUND_ROBIN_GROUPS'].includes(s.stageType)) || !['COPA_A','COPA_B','COPA_TOTAL'].includes(competition.code)}><summary>Configuración general, etapas y Fechas</summary>
